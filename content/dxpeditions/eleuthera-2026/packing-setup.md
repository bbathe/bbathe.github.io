---
date: '2026-03-13T14:03:44-04:00'
draft: false
title: 'Packing & Setup'
summary: "Packing and equipment notes for my Eleuthera 2026 operation."
tags:
  - packing
  - setup
---
With departure approaching quickly, the equipment plan is finalized and everything has been assembled, powered, and is currently operating at home. The emphasis is on reliability, airline survivability, and the ability to operate both from the primary QTH and in the field without reconfiguring the core station.

## Carry on, Pelican 1535
This case contains the core station electronics and anything I am not willing to place in checked baggage.

Icom IC-705 transceiver  
JUMA 1000 amplifier  
Power distribution and essential interconnect cabling

## Personal item, small Aviator bag
This bag supports station control, logging, and digital operation.

Laptop  
RigExpert Stick antenna analyzer  
Heil Proset 7 headset with IC-705 adapter  
PTT foot switch

## Checked luggage, Pelican 1615
This case carries all antenna and site infrastructure for the main station.

Spiderbeam 10 meter mast with guying and stakes  
Antenna elements & radials covering 40m, 20m, 15m, and 10m  
Coaxial feed lines and baluns  
Hand tools and general station support gear 

## POTA field kit
The POTA kit is coming with me, I hope to activate one of the parks on Eleuthera while I'm there.

IC-705 hand mic & earbuds  
Small LiFePO4 battery  
Portable vertical antenna with carbon mast  
Lightweight coax and basic logging materials

## Why this setup
The IC-705 paired with the JUMA 1000 provides an excellent balance of portability and performance for travel based operating. The radio serves double duty as both the primary contest transceiver and the portable POTA rig, reducing redundancy while keeping the system simple. The JUMA 1000 adds the additional output needed for contest operation.

The multiband vertical covering 40m through 10m was selected to focus effort where it matters most for CQ WPX SSB, while remaining fast to deploy and tolerant of limited space. A single vertical with dedicated elements keeps setup time short, minimizes mechanical complexity, and allows quick band changes during the contest.

At this point, the remaining work is logistics rather than engineering.